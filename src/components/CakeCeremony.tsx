...
export const CakeCeremony: React.FC<CakeCeremonyProps> = ({ onComplete }) => {
  ...
  const [stickyNotes] = React.useState<StickyNote[]>([
    { id: 0, text: 'ITS Chawtututu💅🏻!!', x: 20, y: 20 },
    { id: 1, text: 'Hunhh!!', x: window.innerWidth - 140, y: window.innerHeight - 140 },
  ]);
  ...
  return (
    ...
    >
      <div className="text-3xl mb-8 text-pink-600 font-semibold text-center">
        {!isCut ? (
          <span>✨ Maatchis jaise hatho se candles jalado Ehehehe! ✨</span>
        ) : (
          <span>🎉 💗🫶🏻Happyyy Birthdayyy Ninni🫶🏻💗!!! 🎉</span>
        )}
      </div>
      ...
      >
        ...
        {easterEggFound && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
          >
            Bina chaarankh ke dhundliya DAMNNN!!
          </motion.div>
        )}
